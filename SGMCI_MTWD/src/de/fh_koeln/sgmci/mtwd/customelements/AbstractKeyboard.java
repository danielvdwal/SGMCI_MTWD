 ...
 * Since MTKeyboard's and MTTextKeyboard's layout methods are inaccessible, 
 * this class is extending the MTRoundRectangle class to create a new keyboard component,
 * where its layout can be changed by extending this abstract class.<br >
 * Since the layout has its origin from known mobile tablet devices,
 * some constants have been set, to ensure a similarity in the geometry 
 * of the different keyboard layouts, such as the height of the keyboard,
 * margin between buttons, the factor to scale a button, etc..<br >
 * These constants can be ignored in case of a complete new implementation of a keyboard layout.
 * 
...
public abstract class AbstractKeyboard extends MTRoundRectangle {
    ...
    /** The distance between the top of the keyboard and the top of the first row of buttons. */
    static final int MARGIN_TOP = 20;
    /** The distance between the bottom of the keyboard and the bottom of the last row of buttons. */
    ...
    /** The computed total size of a button (includes the inner button size plus 2 times the margin). */
    static final float BUTTON_SIZE = INNER_BUTTON_SIZE + 2 * BUTTON_MARGIN;
    /** 
     * The computed width of a wider button, 
     * (e.g. shift, backspace, change to numbers, etc., 
     * includes a fixed value plus 2 times the margin). */
    ...
    /** The computed width of a return key (includes a fixed value plus 2 times the margin). */
    static final float RETURN_WIDTH = 58 + 2 * BUTTON_MARGIN;
    /** The computed height of the complete keyboard (includes the upper and lower margin). */
    ...
    /** 
     * A map to store all keys as MTSvgButtons and their KeyInfo
     * (includes geometric, id, SVG name information).
     */
    final Map<MTSvgButton, KeyInfo> allKeys;
    /** 
     * A map to store only all keys, that don't change their geometry, 
 as MTSvgButtons and their KeyInfo 
 (includes geometric, id, SVG name information). 
     */
    final Map<MTSvgButton, KeyInfo> normalKeys;
    /** 
     * A map to store only all keys, that are only visible 
     * if the "change to letters" key was pressed, 
     * as MTSvgButtons and their KeyInfo 
     * (includes geometric, id, SVG name information). 
     */
    ...
    /** 
     * A map to store only all keys, that are only visible 
     * if the "change to letters" key and the shift key were pressed, 
     * as MTSvgButtons and their KeyInfo 
     * (includes geometric, id, SVG name information). 
     */
    ...
    /** 
     * A map to store only all keys, that are only visible 
     * if the "change to numbers" key was pressed, 
     * as MTSvgButtons and their KeyInfo 
     * (includes geometric, id, SVG name information). 
     */
    ...
    /** 
     * A map to store only all keys, that are only visible 
     * if the "change to signs" key was pressed, 
     * as MTSvgButtons and their KeyInfo 
     * (includes geometric, id, SVG name information). 
     */
    final Map<MTSvgButton, KeyInfo> signKeys;
    /** The collection of all text input acceptors (the components that receive the text input). */
     ...
     * In the beginning the shift key is not pressed and
     * the uncapitalized letters are shown.
     * 
     ...
     * @param height the height of the keyboard (the constant value only is provided, 
     * in case the constant should be used, its usage needs to be declared explicitly)
     * @param widthArc the width of the arc used for the corners of the keyboard
     * @param heightArc the height of the arc used for the corners of the keyboard
    ...
    /**
     * Changes the visibility of the buttons on the keyboard in regard to the current
     * keyboard visibility mode.
     */
    ...
    /**
     * Shows an animation in which the keyboard shrinks and fades away when the keyboard is closed.
     */
    void closeKeyboard() {
    ...
    /**
     * Creates the MTSvgButton objects by using the given collection of key information
     * and adds them to the given container, as well as the corresponding map.
     * 
     * @param keyInfos the collection of key information to be used to create the MTSvgButton objects
    ...
    /**
     * Retrieve an array of all ITextInputListener objects in the textListener collection.
    ...
    /**
     * Removes the given ITextInputListener object from the textListener collection.
     ...
     */
    enum KeyVisibility {
        /** The key is visible all the time and does not change its geometric information. */
        NORMAL_KEY,
        /** The key is only visible, if the uncapitalized letters-layout is active. */
        LETTERS_IS_SELECTED,
    ...
    /**
     * This class is used to store id, character, SVG file, 
     * visibility and geometry information of a key.<br >
    ...
    class KeyInfo {
        ...
        String svgFileName;
        /** The geometric information for the button (including x and y positions, width and height). */
        Vector3D geometry;
         ...
         * @param charUnicodeToWrite the character(s) to be written, once the button is pressed
         * @param svgFileName the location and name of the SVG file
         * @param geometry the geometric information for the button
         * @param visibilityInfo the layout setup in which the button should be visible
         */
    ...
    /**
     * This class is used to implement an ActionListener, 
     * that, once the shift button has been pressed,
     * performs the necessary actions that take place.
     */
    ...
    /**
     * This class is used to implement an ActionListener, 
     * that, once a key has been pressed,
     * performs the necessary actions that take place.
     */
     ...
     * This class is used to implement an ActionListener, 
     * that, once the shift button has been pressed,
     * in case the button was already in the pressed state,
     * performs the necessary actions that take place.
     */
    ...
    /**
     * This class is used to implement an ActionListener, 
     * that, once the "change to letters" button has been pressed,
     * performs the necessary actions that take place.
     */
    ...
    /**
     * This class is used to implement an ActionListener, 
     * that, once the "change to numbers" button has been pressed,
     * performs the necessary actions that take place.
     */
    ...
    /**
     * This class is used to implement an ActionListener, 
     * that, once the "change to signs" button has been pressed,
     * performs the necessary actions that take place.
     */
